...
const ProductDetails = () => {
    ...
    const [relatedPlroducts, setRelatedProducts] = useState([]);
    ...
    useEffect(()=>{
      if(products.length>0){
        // let productsCopy = products.slice(); // copy
        let productsCopy = products.filter((item)=>item?.category===product?.category);
        productsCopy = productsCopy.filter((product)=>product.inStock).slice(0,5);
        setRelatedProducts(productsCopy);
      }
    },[products]);

    // to set thumbnail product image
    useEffect(()=>{
        ...
        <div className="mt-12">
            ...
            <div className="flex flex-col items-center mt-20">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 md:gap-6 lg:grid-cols-5 mt-6 w-full">
                  {
                    relatedPlroducts.map((product,i)=>{
                      return(
