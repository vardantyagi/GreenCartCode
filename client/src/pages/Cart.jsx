...
const Cart = () => {
    ...
    const { products, currency, updateCartItem, removeFromCart, cartItems, getCartCount, getCartAmount, navigate, removeFromCartCompletely, axios, user, setCartItems } = useAppContext();
    ...
    const placeOrder = async (e) => {
        ...
        try {
            ...
            if (paymentOption === 'COD') {
                const res = await axios.post('/api/order/cod', {
                    // userId: user._id,
            ...
            <div className='flex-1 max-w-4xl'>
                ...
                {cartArray.map((product, index) => (
                    <div key={index} className="grid grid-cols-[2fr_1fr_1fr] text-gray-500 items-center text-sm md:text-base font-medium pt-3">
                        ...
                        <button onClick={() => { removeFromCart(product._id) }} className="cursor-pointer mx-auto"> {/* removeFromCart(product._id) */}
                        ...
                        {showAddress && (
                            <div className="absolute top-12 py-1 bg-white border border-gray-300 text-sm w-full">
                                {addresses && addresses.map((address, i) => {
                                    return (
                                        <p key={i} onClick={() => { setShowAddress(false); setSelectedAddress(address); }} className="text-gray-500 p-2 hover:bg-gray-100">
